...
namespace Rein.BasicPlugins {
	[JsonObject("Rein.BasicPlugins.AddNameFilterConfig")]
	...
	public class AddNameFilter : IPlugin {
		public const string NAME = "Rein.BasicPlugins.AddNameFilter";
		...
		public bool Execute(IBot bot, IMessage message) {
			...
			if (user.Nickname == null || user.Nickname.Length == 0) {
				...
				message.Content = T._("{0} {1}", user.Username, message.Content);
			} else {
				// TRANSLATORS: Bot message. AddNameFilter plugin. {0} is user name. {1} is message content.
				message.Content = T._("{0} {1}", user.Nickname, message.Content);
			}
