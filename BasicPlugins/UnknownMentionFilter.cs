...
namespace Rein.BasicPlugins {
	public class UnknownMentionFilter : IPlugin {
		public string Name { get; } = "Rein.BasicPlugins.UnknownMentionFilter";
		...
		public bool Execute(IBot bot, IMessage message) {
			...
			if (!Rein.Utility.Messages.IsMentionFor(bot.Id, message.Original)) { return false; }
			if (message.Original.Author.IsBot) { return false; }
			if (message.AppliedPlugins.Contains("Rein.UserDefinedFilterPlugin.UserDefinedFilter")) { return false; }
