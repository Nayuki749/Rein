...
namespace Rein.UserDefinedFilterPlugin {
	public class UserDefinedFilter : IPlugin {
		public string Name { get; } = "Rein.UserDefinedFilterPlugin.UserDefinedFilter";
		...
		private void Send(IBot bot, SocketMessage message, string text) {
			if (text.Length == 0) { return; }
			var msg = Rein.Plugins.Utility.ReplaceDiscordValues(text, bot, DateTime.Now, message);
		...
		public bool Execute(IBot bot, IMessage message) {
			...
			foreach (var filter in this.filters) {
				var result = filter.DoFilter(bot, message);
				if (result.Content != null) {
					message.Content = result.Content;
					applied = true;
				}
				if (result.SendMessage != null && result.SendMessage.Length > 0) {
					Send(bot, message.Original, result.SendMessage);
					applied = true;
				}
			}
			if (applied) {
