...
namespace Rein.Plugins.Schedules {
	...
	public abstract class ScheduledTaskBase : IScheduledTask {
		...
		private string[] ReplaceContent(string content, IBot bot, DateTime now) {
			var text1 = Rein.Plugins.Utility.ReplaceDiscordValues(content, bot, now);
			var text2 = Rein.Plugins.Utility.ReplaceDiscordValues(content.Replace("${announce}", "").Replace("${speak}", ""), bot, now);
